...
> Codex loading note: `paths:` is consumed by `.codex/scripts/resolve_policies.py`; Codex CLI does not natively apply this frontmatter.
...
Codex CLI must never simulate, infer, fabricate, self-grant, or backfill approval. Proposal and approval storage must remain distinct from active configuration and authoritative runtime state. Tests must cover replay, hash mismatch, stale approval, scope mismatch, missing approver, and attempted direct activation.
