...
SENSITIVE = re.compile(
    r"(?i)(api[_-]?key|authorization|credential|password|secret|token|private[_-]?key|cookie)"
)
TELEMETRY_FIELDS = {
    "schema_version",
    "timestamp",
...
try:
    ...
    err(f"config TOML invalid: {e}")
agent_names = set()
for p in sorted((ROOT / ".codex/agents").glob("*.toml")):
    try:
        d = tomllib.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        err(f"{p.relative_to(ROOT)} invalid TOML: {e}")
        continue
...
for d in sorted((ROOT / ".agents/skills").iterdir()):
    ...
    if schema.is_file():
        try:
            ...
            if sd.get("properties", {}).get("skill", {}).get("const") != name:
                err(f"{schema.relative_to(ROOT)} skill does not match {name}")
            events = sd.get("properties", {}).get("event", {}).get("enum", [])
            if not events or any(e in {"tool_used", "step_completed"} for e in events):
                err(f"{schema.relative_to(ROOT)} has generic or empty event names")
            if set(sd.get("properties", {})) - TELEMETRY_FIELDS:
                err(f"{schema.relative_to(ROOT)} contains undeclared telemetry fields")
            if any(SENSITIVE.search(k) for k in sd.get("properties", {})):
                err(f"{schema.relative_to(ROOT)} contains sensitive field name")
        except Exception as e:
            err(f"{schema.relative_to(ROOT)} invalid JSON: {e}")
...
for base in [ROOT / "AGENTS.md", ROOT / ".agents", ROOT / ".codex/agents", ROOT / ".codex/memory"]:
    ...
    for p in files:
        ...
        if re.search(
            r"(?<![A-Za-z0-9_-])(?:[a-z0-9-]+-(?:engineer|reviewer|verifier)|phase-integrator)\.md\b",
            t,
        ):
            err(f"stale markdown agent path remains: {p.relative_to(ROOT)}")

# Explicit references to local scripts, agents, skills, and policy files must resolve.
for p in [
    ROOT / ".agents/skills",
    ROOT / ".codex/agents",
    ROOT / ".codex/memory",
    ...
    for f in p.rglob("*"):
        if not f.is_file() or f.suffix not in {".md", ".toml"}:
            continue
        t = f.read_text(encoding="utf-8", errors="ignore")
        for ref in re.findall(
            r"(?<![\w./-])((?:\.agents|\.codex)/(?:scripts|agents|skills|memory)/[A-Za-z0-9_./-]+)",
            t,
        ):
