...
2. Run `python .codex/scripts/resolve_policies.py <changed-paths...>` and use the returned policy set as the exact path-scope map. For the safety categories below, cross-check the applicable policies, especially: `.codex/policies/kalshi-transport-safety.md`, `.codex/policies/credential-privacy.md`, `.codex/policies/agents/openrouter-governance.md` plus `.codex/policies/governance-and-approvals.md`, and `.codex/policies/architecture/dependency-boundaries.md`. Treat a category as in-scope if any changed file, new schema field, or new config matches its pattern or purpose.
