...
7. Resolve path-sensitive policy for the candidate write set with `python .codex/scripts/resolve_policies.py <paths...>` and read every returned policy. Re-run the resolver if the write set expands.
8. Enumerate affected safety invariants, authority boundaries, persistence contracts, schemas, migrations, public interfaces, observability, failure modes, and tests.
9. Confirm unrelated worktree changes will not be overwritten.
10. Define the narrowest coherent implementation boundary and later-phase behavior that must remain unimplemented.
11. Produce the report below.
